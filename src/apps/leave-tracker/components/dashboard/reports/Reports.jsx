...
function Reports() {
  ...
  const yearList = useMemo(() => createYearList(10), []);
  ...
  const initialDataState = useMemo(
    ...
    [currentDate, monthsList, yearList]
  );
  ...
  const handleCheckSections = ({ target: input }) => {
    if (input.checked) {
      ...
      const index = sectionData.findIndex((section) => section === input.value);
      sectionData.splice(index, 1);
      setSectionList(sectionData);
    }
  };
  const handleChangeType = ({ target: input }) => setType(input.value);
  const handleChangeData = ({ target: input }) => setData({ ...data, [input.name]: input.value });
  ...
  const getLeaveCountLabel = () => <span>Leave count as of {getLeaveCountLabelPeriod()}</span>;
    ...
    <div className="report">
      ...
      <main className="grid gap--10px">
        ...
        <div className="report__section">
          <p className="info-label">Select sections to print in report:</p>
          <div className="grid grid--1x2 grid--tablet-hr">
            <CheckBox
              label={getLeavesLabel()}
              value={1}
              id={1}
              handleChange={handleCheckSections}
