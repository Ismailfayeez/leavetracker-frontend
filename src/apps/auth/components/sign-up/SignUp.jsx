...
function SignUp() {
  ...
  const [errors, validateForm, validateProperty] = useValidator(data, signupSchema);
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        const { data: jwt } = await dispatch(login({ email: data.email, password: data.password }));
