...
function ProjectSectionView({
  ...
  const [sectionData, handleDelete, handleClose, moveToNextPage] =
    useProjectSectionView(sectionConstants, sectionId);
  ...
  if (!fields)
    fields = Object.keys(data).map((key) => ({ name: key, label: key }));
  console.log(data);
  return (
