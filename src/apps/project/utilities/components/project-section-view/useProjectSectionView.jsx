...
import { closeSectionDetail, deleteSectionDetail } from '../../../store/projects';
...
export const useProjectSectionView = (sectionConstants, sectionId) => {
  ...
  const sectionData = useSelector((state) => state.entities.projects.core[name]);
  ...
  const handleRemoveData = async (methodType) => {
    ...
    toastMessage = `${name} ${primaryFieldVal} ${methodType === 'close' ? 'closed' : 'deleted'}
  ${' '} successfully`;
  ...
  return [sectionData, handleDelete, handleClose, moveToNextPage, moveToPrevPage];
};
