...
export const employeeWithRoleValidation = (Component, employeeData, allowedAccessList = []) => {
  if (employeeData.isLoading) return <LoadingScreen />;
  if (employeeData.id) {
    if (employeeData.role && employeeData.role.access) {
      const employeeAccessList = employeeData.role.access.map((accessObj) => accessObj.code);
