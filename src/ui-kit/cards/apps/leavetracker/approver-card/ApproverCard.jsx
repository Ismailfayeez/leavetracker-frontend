...
function ApproverCard({ approver, enableRemoveBtn, handleRemove, loading }) {
  return (
    <div className={`approver card approver--pista-green`}>
      <div className="card__body flex flex--center gap--0_5rem">
        <div className="profile-img-container">
          <div className="profile-img">{approver.username.slice(0, 1)}</div>
        </div>
        <div className="flex-grow overflow-auto">
          <div className="approver__name bold text-overflow-ellipsis text-transform-capitalize">
            {approver.username}
          </div>
          <div className="approver__email sub-text text-overflow-ellipsis">
            {approver.email}
          </div>
        </div>
        {enableRemoveBtn && (
